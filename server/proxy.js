...
// Import migration service (we need to convert it to ESM format)
import { setupMigrationEndpoint, migrations, processMigration, performMigration, getApiVersionForKind } from './migration-service.js';
...
function determineResourceApiPath(kind, namespace, name) {
  // API paths for different resource types
  const apiPathMap = {
...
app.post('/kube-migrate/k8s/migrate', async (req, res) => {
  ...
  try {
    // Process the migration with options
    const migrationOptions = {
      ...options,
      // Provide detailed diagnostics for debugging
      debugMode: options?.debugMode === true || process.env.DEBUG === 'true'
    };

    // Check configs before starting
    try {
      // Validate source kubeconfig by trying to access the server
      console.log('Validating source cluster connection...');
      const sourceTest = await makeK8sRequestWithRetry(sourceKubeconfig, '/api/v1', 'GET');
      console.log('Source cluster connection validated successfully');
      
      // Validate target kubeconfig
      console.log('Validating target cluster connection...');
      const targetTest = await makeK8sRequestWithRetry(targetKubeconfig, '/api/v1', 'GET');
      console.log('Target cluster connection validated successfully');
    } catch (validationError) {
      console.error('Cluster validation failed:', validationError);
      
      // Create initial migration status with warning about validation failure
      const initialStatus = {
        id: migrationJobId,
        status: 'in-progress',
        progress: 0,
        ...
        failedResources: [],
        logs: [
          { timestamp: new Date(), level: 'warning', message: `Cluster validation failed: ${validationError.error}` },
          { timestamp: new Date(), level: 'info', message: 'Continuing with migration attempt, but it may fail' }
        ],
        isSimulated: false,
        warning: `Cluster validation failed: ${validationError.error}. Migration may fail.`
      };
      
      // Store initial status
      migrations.set(migrationJobId, initialStatus);
    }
    
    // Process migration in background thread
    (async () => {
      try {
        await processMigration(migrationJobId, sourceKubeconfig, targetKubeconfig, resources, migrationOptions);
      } catch (error) {
        console.error(`Background migration processing failed: ${error.message}`);
      }
    })();
    
    // Respond immediately with success and the migration ID
    return res.json({ 
      status: 'success', 
    ...
    const failedStatus = {
      ...
      logs: [
        { timestamp: new Date(), level: 'error', message: `Migration failed to start: ${error.message}` }
      ],
      isSimulated: false,
      warning: `Migration process failed to start: ${error.message}.`
    };
    ...
    return res.status(500).json({ 
      status: 'error', 
      migrationId: migrationJobId,
      message: `Migration failed to start: ${error.message}`
    });
  }
});
