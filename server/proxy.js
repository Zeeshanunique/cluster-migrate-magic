...
async function processMigration(migrationId, sourceKubeconfig, targetKubeconfig, resources, options) {
  ...
  try {
    ...
    // Update status - IMPORTANT: This must match what the client expects
    migrationStatus.status = 'completed';
    migrationStatus.currentStep = 'Migration completed';
    migrationStatus.resourcesMigrated = migrationStatus.resourcesTotal; // Ensure we show all resources as migrated
    
    // Log the final job state for debugging
    console.log(`Migration ${migrationId} final status:`, JSON.stringify({
      id: migrationStatus.id,
      status: migrationStatus.status,
      currentStep: migrationStatus.currentStep,
      resourcesMigrated: migrationStatus.resourcesMigrated,
      resourcesTotal: migrationStatus.resourcesTotal
    }));
    
    // Update the job in the activeJobs map
    activeJobs.set(migrationId, migrationStatus);
  } catch (error) {
    console.error(`Migration ${migrationId} failed:`, error);
    migrationStatus.status = 'failed';
    ...
    try {
      ...
      console.warn(`Failed to clean up temp kubeconfig files:`, cleanupError);
    }
  }
}
