...
const MultiCluster = () => {
  ...
  useEffect(() => {
    const loadClusters = async () => {
      ...
      try {
        console.log('Loading multi-clusters for user:', user.id);
        const data = await clusterService.getAllClusters(user.id);
        // Filter to only show multi-type clusters
        const multiClusters = data.filter(cluster => cluster.type === 'multi');
        setClusters(multiClusters);
        console.log('Multi-clusters loaded successfully:', multiClusters.length);
      } catch (error) {
        console.error('Error loading multi-clusters:', error);
        toast.error('Failed to load multi-clusters');
        setConnectionError('Could not retrieve multi-clusters. Please check your Supabase setup.');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handleRetry = () => {
    if (user) {
      ...
      clusterService.getAllClusters(user.id)
        ...
        .catch(error => {
          console.error('Error retrying multi-cluster load:', error);
          toast.error('Still unable to load multi-clusters');
        ...
        <div className="container px-4 mx-auto py-8">
          <div className="mb-8 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              ...
              <p className="text-muted-foreground mt-2">
                Manage your Kubernetes multi-cluster environments
              </p>
            </div>
          ...
          <div className="mb-8 grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-white dark:bg-gray-800 rounded-lg p-6 shadow-subtle border border-gray-200 dark:border-gray-700">
              <div className="flex items-center">
                ...
                <div>
                  <h3 className="text-lg font-medium mb-1">Total Multi-Clusters</h3>
          ...
          <div className="mb-6">
            <div className="relative flex-grow max-w-md">
              ...
              <Input
                placeholder="Search multi-clusters..."
                className="pl-10"
                value={searchQuery}
          ...
          {isLoading ? (
            ...
            <div className="text-center py-12 bg-gray-50 dark:bg-gray-800/50 rounded-lg border border-gray-200 dark:border-gray-700">
              ...
              <p className="text-lg text-muted-foreground mb-4">No multi-clusters found</p>
              <p className="text-muted-foreground max-w-md mx-auto mb-6">
                Multi-clusters enable you to manage workloads across multiple Kubernetes environments. 
                Create one or migrate an existing single cluster.
              </p>
              <Button 
                variant="outline" 
