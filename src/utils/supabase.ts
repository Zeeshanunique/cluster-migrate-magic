...
export const clusterService = {
  ...
  async convertToMultiCluster(
    ...
    try {
      ...
      console.log(`Converting cluster ${sourceCluster.name} (${sourceClusterId}) to multi-cluster`);
      
      // Check if this cluster is already converted to prevent duplication
      if (sourceCluster.type === 'multi') {
        console.log(`Cluster ${sourceCluster.name} is already a multi-cluster, skipping conversion`);
        toast.info(`Cluster "${sourceCluster.name}" is already a multi-cluster`);
        return true;
      }
