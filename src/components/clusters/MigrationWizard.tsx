...
const MigrationWizard = () => {
  ...
  useEffect(() => {
    const loadClusters = async () => {
      ...
      try {
        ...
        const multiClusters = clusters.filter(c => c.type === 'multi');
        
        setAvailableSingleClusters(singleClusters);
        setAvailableMultiClusters(multiClusters);
        ...
        // Auto-select the first multi-cluster as target if available
        if (multiClusters.length > 0) {
          handleTargetClusterSelect(multiClusters[0].id);
        }
      } catch (error) {
  ...
  const startMigration = async () => {
    ...
    try {
      // Prevent duplicate migration if source is already a multi-cluster
      if (sourceCluster?.type === 'multi') {
        toast.warning(`Cluster "${sourceCluster.name}" is already a multi-cluster`);
        setStatus('error');
        setError("Source cluster is already a multi-cluster");
        return;
      }
    ...
    const selectedPVs = persistentVolumes.filter(pv => pv.selected);
      ...
      if (migrationSuccessful) {
        ...
        if (sourceCluster && user?.id) {
          ...
          const success = await clusterService.convertToMultiCluster(sourceCluster.id, {
          ...
          <div className="space-y-6 py-4">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Select a multi-cluster as your migration target
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-6">
                {loadingClusters ? (
                  ...
                  <div className="bg-muted p-4 rounded-md text-center">
                    <p className="text-muted-foreground mb-2">No multi-clusters available as target</p>
                    <p className="text-sm text-muted-foreground mb-4">You need at least one multi-cluster for migration.</p>
                    <Button 
                      variant="outline" 
                      size="sm" 
                      onClick={() => navigate('/create-cluster')}
                    ...
                    <div className="space-y-2">
                      ...
                      >
                        <SelectTrigger id="target-cluster" className="w-full">
                          <SelectValue placeholder="Select a multi-cluster" />
                        </SelectTrigger>
                        <SelectContent>
              ...
              <CardContent className="space-y-6">
                ...
                <div className="bg-green-50 dark:bg-green-950/30 p-4 rounded-md">
                  ...
                  <ul className="space-y-2 text-sm text-green-700 dark:text-green-300">
                    ...
                    <li className="flex items-center">
                      <Check className="h-4 w-4 mr-2" />
                      Updated cluster type from single to multi
                    </li>
                    <li className="flex items-center">
                      <Check className="h-4 w-4 mr-2" />
